...
namespace Tipoul.Services.Shahins.WebApi.Controllers
{
    ...
    public class AccountStatementController : ControllerBase
    {
        ...
        public AccountStatementController(ILogger<AccountStatementController> logger, IConfiguration iConfig)
        {
            _logger = logger;
        ...
        [HttpGet()]
        public async Task<AccountStatementResult?> Get()
        {
            ...
            AccountStatement _i = new AccountStatement();
            _i.bank = tokenResult.bank;
            _i.nationalCode = tokenResult.user_name;
            _i.sourceAccount = tokenResult.accounts[0];

            GetAccountStatement a = new GetAccountStatement();
            Framework.Services.OpenBanking.Shahin.Inquiry.Account.Models.AccountStatementResult accountStatementResult = await a.GetAccountStatements(_i, ApiUrl, UserName, Password, tokenResult.access_token, "");
